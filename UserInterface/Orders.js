...
import { TruckId, Images, DetailPopup } from './UpdatedData';
...
export default function Orders({ route, navigation }) {
  ...
  const getImage = (item) => {
    var index = Number(String(item[0]).slice(-2))
    if (item[5] == 'Female') {
      return index > 50 ? femaleImages[index - 50] : femaleImages[index];
    }
    else if (item[5] == 'Male') {
      return index >= 50 ? maleImages[index - 50] : maleImages[index];
    }
    else {
  ...
  useEffect(() => {
    navigation.addListener('focus', () => {
      ...
      // if ((currentTime >= 0 && currentTime < 9) || (currentTime >= 15 && currentTime < 17)) {
      //   setIsTruckInTransit(true);
      //   setNewTruckId();
      // }
      // else {
  ...
  const InQueue = () => (
    <View style={Styles.container}>
      {femaleImages.length > 0 && maleImages.length > 0 && placeHolderImg.length > 0 && isInqueueDataFetched &&
        <FlatList
          data={inQueueData}
          scrollEnabled={true}
          vertical
  ...
  const OrderHistory = () => (
    <View style={Styles.container}>
      {femaleImages.length > 0 && maleImages.length > 0 && placeHolderImg.length > 0 && isHistoryDataFetched &&
        <FlatList
